...
def generate_collage(df, mood_fallback:str, seed:int=42):
    rows = min(len(df), 7)
    ...
    for i in range(rows):
        ...
        piece = generate_art(seed+i, steps, hr, sleep, fatigue, mood, canvas_size=(600,400))
        ax = fig.add_subplot(2,4,i+1)
        ax.imshow(piece.canvas.renderer.buffer_rgba()); ax.axis('off')
...
with col1:
    ...
    if collage_on and df is not None and len(df) >= 2:
        ...
        fig = generate_art(img_seed, steps_today if df is None else int(df.iloc[-1].get('steps', steps_today) or steps_today),
                           hr_avg if df is None else float(df.iloc[-1].get('heart_rate_avg', hr_avg) or hr_avg),
                           sleep_hours if df is None else float(df.iloc[-1].get('sleep_hours', sleep_hours) or sleep_hours),
                           fatigue if df is None else float(df.iloc[-1].get('fatigue', fatigue) or fatigue),
                           mood if df is None else str(df.iloc[-1].get('mood', mood) or mood))
        st.pyplot(fig, use_container_width=True)
...
with col2:
    ...
    st.markdown("""
- **Mood → Colors**: palette
- **Steps → Complexity**: number of shapes/bubbles
- **Heart Rate → Flow/Jitter**: curvature and agitation
- **Sleep → Softness/Transparency**: layer softness
- **Fatigue → Density/Line Weight**: denser/darker strokes
""")
st.markdown('---')
st.caption("CSV optional; try 'Use Sample' to see a demo.")
